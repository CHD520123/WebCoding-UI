...
function createAgentRuntime(deps) {
  ...
  function sendRuntimeMessage(entry, sessionId, payload) {
    if (typeof broadcastSessionMessage === 'function' && sessionId) {
      broadcastSessionMessage(sessionId, payload);
      return;
    }
    if (entry?.controllerWs) {
      wsSend(entry.controllerWs, payload);
      return;
    }
    wsSend(entry?.ws, payload);
  }
  ...
  function ensureCodexToolCall(entry, item, sessionId = null) {
    ...
    sendRuntimeMessage(entry, sessionId, {
      type: 'tool_start',
      name: tc.name,
      toolUseId: item.id,
      input: tc.input,
    ...
    switch (event.type) {
      ...
      case 'assistant': {
        ...
        for (const block of content) {
          if (block.type === 'text' && block.text) {
            ...
            sendRuntimeMessage(entry, sessionId, { type: 'text_delta', text: block.text });
            ...
            entry.toolCalls.push(tc);
            appendEntryToolSegment(entry, tc);
            sendRuntimeMessage(entry, sessionId, { type: 'tool_start', name: block.name, toolUseId: block.id, input: tc.input });
            ...
            sendRuntimeMessage(entry, sessionId, { type: 'tool_end', toolUseId: block.tool_use_id, result: truncatedResult });
          }
        }
      ...
      case 'result': {
        ...
        if (event.total_cost_usd !== undefined) {
          sendRuntimeMessage(entry, sessionId, { type: 'cost', costUsd: entry.claudeSessionTotalCost || 0 });
        }
        break;
      }
    }
    ...
    switch (event.type) {
      ...
      case 'item.started': {
        ...
        if (item.type === 'agent_message') {
          if (!entry.codexStreamingItems) entry.codexStreamingItems = {};
          entry.codexStreamingItems[item.id] = { sentLen: 0 };
          break;
        }
      ...
      case 'item.updated': {
        ...
        if (!text) break;
        if (!entry.codexStreamingItems) entry.codexStreamingItems = {};
        const state = entry.codexStreamingItems[item.id] || { sentLen: 0 };
        entry.codexStreamingItems[item.id] = state;
        const delta = text.slice(state.sentLen);
        if (delta) {
          state.sentLen = text.length;
          sendRuntimeMessage(entry, sessionId, { type: 'text_delta', text: delta });
        }
        break;
      }
      ...
      case 'item.completed': {
        ...
        if (item.type === 'agent_message') {
          const text = item.text || '';
          const sentLen = entry.codexStreamingItems?.[item.id]?.sentLen || 0;
          const remaining = text.slice(sentLen);
          if (remaining) {
            sendRuntimeMessage(entry, sessionId, { type: 'text_delta', text: remaining });
          }
          if (text) {
            entry.fullText += text;
            appendEntryTextSegment(entry, text);
          }
          if (entry.codexStreamingItems) delete entry.codexStreamingItems[item.id];
          break;
        }
        if (item.type === 'error') {
          const warningMessage = formatCodexItemErrorMessage(item);
          if (warningMessage) {
            sendRuntimeMessage(entry, sessionId, { type: 'system_message', message: warningMessage });
          }
          break;
        }
      ...
      case 'turn.completed': {
        ...
        if (session && usage) {
          ...
          saveSession(session);
          sendRuntimeMessage(entry, sessionId, { type: 'usage', totalUsage: session.totalUsage });
        }
        break;
      }
      ...
      case 'error':
        if (event.message) {
          if (/^Reconnecting\.\.\./.test(event.message)) {
            sendRuntimeMessage(entry, sessionId, { type: 'system_message', message: event.message });
          } else {
