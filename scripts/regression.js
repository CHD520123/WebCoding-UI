...
function runSqliteScriptWithNodeSqlite(dbPath, script) {
  let DatabaseSync;
  try {
    ({ DatabaseSync } = require('node:sqlite'));
  } catch (error) {
    throw new Error(`sqlite3 command not found and node:sqlite is unavailable: ${error.message}`);
  }

  const trimmed = String(script || '').trim();
  const bodyWithoutFinalSemicolon = trimmed.replace(/;\s*$/, '');
  const isSingleStatement = !/;\s*\S/.test(bodyWithoutFinalSemicolon);
  const isQuery = isSingleStatement && /^(select|pragma)\b/i.test(trimmed);
  const db = new DatabaseSync(dbPath, {
    timeout: 1000,
    ...(isQuery ? { readOnly: true } : {}),
  });
  try {
    if (isQuery) {
      ...
      return rows
        .map((row) => columns.map((column) => row[column] == null ? '' : String(row[column])).join('|'))
        .join('\n');
    }
    db.exec(script);
    return '';
  } catch (error) {
    if (isQuery && /database is locked/i.test(error.message || '')) return '__LOCKED__';
    throw error;
  } finally {
    db.close();
  }
...
function runSqliteScript(dbPath, script) {
  ...
  if (result.error?.code === 'ENOENT') {
    return runSqliteScriptWithNodeSqlite(dbPath, script);
  }
  if (result.status !== 0) {
...
function createFakeSkillFixtures(homeDir) {
  const skillsDir = path.join(homeDir, '.codex', 'skills');
  mkdirp(skillsDir);
  mkdirp(path.join(skillsDir, 'maestro-quick'));
  mkdirp(path.join(skillsDir, 'manage-status'));
  mkdirp(path.join(skillsDir, '.system-hidden'));
  fs.writeFileSync(path.join(skillsDir, 'maestro-quick', 'SKILL.md'), `---
name: maestro-quick
description: Fast-track single task execution with workflow guarantees
argument-hint: "\"task description\""
---
`);
  fs.writeFileSync(path.join(skillsDir, 'manage-status', 'SKILL.md'), `---
name: manage-status
description: Display project dashboard with phase progress
argument-hint: ""
---
`);
  fs.writeFileSync(path.join(skillsDir, '.system-hidden', 'SKILL.md'), '# ignored');
}
...
async function runSessionControlSyncRegressionCase({ port, password }) {
  let connectionA = null;
  let connectionB = null;
  try {
    connectionA = await connectAuthedClient(port, password);
    const session = await connectionA.client.sendAndWaitType(
      buildAgentMessagePayload({ text: 'trigger codex slow stream handoff', mode: 'yolo', agent: 'codex' }),
      'session_info',
      (msg) => msg.agent === 'codex' && msg.title === 'trigger codex slow stream handoff',
    );
    assert(session.controlState === 'viewer' || session.controlState === 'controller' || session.controlState === null, 'Initial session_info should remain backward compatible');
    await connectionA.client.waitForType('text_delta', (msg) => /slow-start:handoff/.test(msg.text || ''), 5000);

    connectionB = await connectAuthedClient(port, password);
    connectionB.client.send({ type: 'load_session', sessionId: session.sessionId });
    const loaded = await connectionB.client.waitForType(
      'session_info',
      (msg) => msg.sessionId === session.sessionId && msg.isRunning === true,
      5000,
    );
    assert(loaded.controlState === 'viewer', 'Second viewer should load running session in viewer mode');
    assert(loaded.viewerCount === 2, 'Second viewer should see both viewers connected');
    assert(loaded.controllerClientId, 'Running session should expose a controller client id');
    await connectionB.client.waitForType(
      'resume_generating',
      (msg) => msg.sessionId === session.sessionId && /slow-start:handoff/.test(msg.text || ''),
      5000,
    );

    const controllerStateA = await connectionA.client.waitForType(
      'control_changed',
      (msg) => msg.sessionId === session.sessionId && msg.controlState === 'controller' && msg.viewerCount === 2,
      5000,
    );
    assert(controllerStateA.controllerClientId, 'Initial controller state should include controller client id');

    connectionB.client.send(buildAgentMessagePayload({
      text: 'observer takeover attempt',
      sessionId: session.sessionId,
      mode: 'yolo',
      agent: 'codex',
    }));

    const controllerChangedForA = await connectionA.client.waitForType(
      'control_changed',
      (msg) => msg.sessionId === session.sessionId && msg.controlState === 'viewer' && msg.viewerCount === 2,
      5000,
    );
    const controllerChangedForB = await connectionB.client.waitForType(
      'control_changed',
      (msg) => msg.sessionId === session.sessionId && msg.controlState === 'controller' && msg.viewerCount === 2,
      5000,
    );
    assert(
      controllerChangedForA.controllerClientId && controllerChangedForA.controllerClientId === controllerChangedForB.controllerClientId,
      'Control handoff should converge on the same controller client id for both viewers',
    );
    const busyError = await connectionB.client.waitForType(
      'error',
      (msg) => /正在处理中/.test(msg.message || ''),
      5000,
    );
    assert(/正在处理中/.test(busyError.message || ''), 'Takeover message during active run should still preserve busy guard');

    await closeWs(connectionA.ws);
    await connectionB.client.waitForType('text_delta', (msg) => /slow-mid:handoff|slow-end:handoff/.test(msg.text || ''), 5000);
    await connectionB.client.waitForType('done', (msg) => msg.sessionId === session.sessionId, 10000);
  } finally {
    if (connectionA) await closeWs(connectionA.ws);
    if (connectionB) await closeWs(connectionB.ws);
  }
...
async function runSkillCommandsRegressionCase({ port, password }) {
  await withAuthedClient(port, password, async ({ client }) => {
    const result = await client.sendAndWaitType(
      { type: 'get_skill_commands' },
      'skill_commands',
    );
    const commands = Array.isArray(result.commands) ? result.commands : [];
    assert(commands.some((entry) => entry.cmd === '$maestro-quick' && /Fast-track/.test(entry.desc || '')), 'Skill command list should include installed maestro skills');
    assert(commands.some((entry) => entry.cmd === '$manage-status' && /Display project dashboard/.test(entry.desc || '')), 'Skill command list should include installed non-maestro skills');
    assert(!commands.some((entry) => /\.(system|hidden)/.test(entry.cmd || '')), 'Skill command list should not include hidden/system directories');
  });
}
...
async function runHappyPathRegressionCase({ port, password, tempRoot, configDir, sessionsDir, logsDir, homeDir, codexFixture, tinyPng }) {
  await withAuthedClient(port, password, async ({ client, token }) => {
    ...
    const claudeSlashHelp = await client.sendAndWaitType(
      buildAgentMessagePayload({ text: '/help', sessionId: claudeModelSession.sessionId, mode: 'yolo', agent: 'claude' }),
      'system_message',
      (msg) => /其他 Claude Code 原生指令将直接转发至 CLI/.test(msg.message || ''),
    );
    assert(/其他 Claude Code 原生指令将直接转发至 CLI/.test(claudeSlashHelp.message || ''), 'Claude /help should mention native slash command forwarding');

    client.send(buildAgentMessagePayload({ text: '/memory', sessionId: claudeModelSession.sessionId, mode: 'yolo', agent: 'claude' }));
    await client.waitForType('text_delta', (msg) => /Claude mock handled: \/memory/.test(msg.text || ''), 8000);
    await client.waitForType('done', (msg) => msg.sessionId === claudeModelSession.sessionId);

    client.send(buildAgentMessagePayload({ text: '/effort high', sessionId: claudeModelSession.sessionId, mode: 'yolo', agent: 'claude' }));
    await client.waitForType('text_delta', (msg) => /Claude mock handled: \/effort high/.test(msg.text || ''), 8000);
    await client.waitForType('done', (msg) => msg.sessionId === claudeModelSession.sessionId);

    client.send(buildAgentMessagePayload({ text: '/status', sessionId: claudeModelSession.sessionId, mode: 'yolo', agent: 'claude' }));
    await client.waitForType('text_delta', (msg) => /Claude mock handled: \/status/.test(msg.text || ''), 8000);
    await client.waitForType('done', (msg) => msg.sessionId === claudeModelSession.sessionId);

    client.send(buildAgentMessagePayload({ text: '/commit', sessionId: claudeModelSession.sessionId, mode: 'yolo', agent: 'claude' }));
    await client.waitForType('text_delta', (msg) => /Claude mock handled: \/commit/.test(msg.text || ''), 8000);
    await client.waitForType('done', (msg) => msg.sessionId === claudeModelSession.sessionId);

    const storedClaudeSlashSession = JSON.parse(fs.readFileSync(path.join(sessionsDir, `${claudeModelSession.sessionId}.json`), 'utf8'));
    assert(!storedClaudeSlashSession.messages.some((entry) => entry.role === 'user' && /^\/(?:memory|effort high|status|commit)$/.test(entry.content || '')), 'Forwarded Claude slash commands should stay hidden from user history');

    const codexUnknownSlash = await client.sendAndWaitType(
      buildAgentMessagePayload({ text: '/memory', sessionId: codexSession.sessionId, mode: 'plan', agent: 'codex' }),
      'system_message',
      (msg) => /未知指令: \/memory/.test(msg.message || ''),
    );
    assert(/未知指令: \/memory/.test(codexUnknownSlash.message || ''), 'Codex-only exec mode should not forward TUI slash commands');

    const claudeUnknownWithoutSession = await client.sendAndWaitType(
      buildAgentMessagePayload({ text: '/memory', mode: 'yolo', agent: 'claude' }),
      'system_message',
      (msg) => /未知指令: \/memory/.test(msg.message || ''),
    );
    assert(/未知指令: \/memory/.test(claudeUnknownWithoutSession.message || ''), 'Claude slash forwarding should require an existing session');
...
async function main() {
  ...
  try {
    ...
    createFakeSkillFixtures(homeDir);
    ...
    await withServer(serverEnv, async (serverHandle) => {
      ...
      await runner.run('skill command listing', () => runSkillCommandsRegressionCase(ctx));
      ...
      await runner.run('session control sync and takeover', () => runSessionControlSyncRegressionCase(ctx));
