...
function parseSkillFrontmatter(raw) {
  ...
  for (const line of match[1].split(/\r?\n/)) {
    ...
    if (!item) continue;
    frontmatter[item[1]] = item[2].trim().replace(/^"(.*)"$/, '$1');
  }
  return frontmatter;
}

function listInstalledSkillCommands() {
  if (!USER_HOME || !fs.existsSync(CODEX_SKILLS_DIR)) return [];
  try {
    return fs.readdirSync(CODEX_SKILLS_DIR, { withFileTypes: true })
      ...
      .sort((a, b) => a.cmd.localeCompare(b.cmd, 'en'));
  } catch {
    return [];
  }
}
...
function isAllowedAppOrigin(origin, req = null) {
  ...
  if (parsedOrigin.protocol === 'capacitor:' && parsedOrigin.hostname === 'localhost') {
    return true;
  }
...
function getAllowedCorsOrigin(req) {
  ...
  if (!origin) return '';
  return isAllowedAppOrigin(origin, req) ? origin : '';
}
...
function applyCorsHeaders(req, headers = {}) {
  ...
  if (allowedOrigin) {
    nextHeaders['Access-Control-Allow-Origin'] = allowedOrigin;
    nextHeaders['Access-Control-Allow-Credentials'] = 'true';
    nextHeaders['Access-Control-Allow-Headers'] = 'Content-Type, Authorization, X-Filename';
    nextHeaders['Access-Control-Allow-Methods'] = 'GET, POST, DELETE, OPTIONS';
    nextHeaders.Vary = appendVaryHeader(nextHeaders.Vary, 'Origin');
  }
  return nextHeaders;
}

function jsonResponseWithCors(req, res, statusCode, payload) {
  writeHeadWithSecurity(res, statusCode, applyCorsHeaders(req, {
...
// Active processes: sessionId -> { pid, viewers, controllerWs, controllerClientId, fullText, toolCalls, segments, lastCost, tailer }
const activeProcesses = new Map();

// Track which session each ws is viewing: ws -> sessionId
const wsSessionMap = new Map();

// Track stable client ids for each websocket: ws -> clientId
const wsClientMap = new Map();
...
function setActiveProcess(sessionId, entry) {
  if (!(entry.viewers instanceof Set)) entry.viewers = new Set();
  if (entry.controllerWs && !entry.viewers.has(entry.controllerWs)) {
    entry.viewers.add(entry.controllerWs);
  }
  if (entry.controllerWs && !entry.controllerClientId) {
    entry.controllerClientId = wsClientMap.get(entry.controllerWs) || null;
  }
...
function getLiveViewerSet(entry) {
  if (!entry) return new Set();
  if (!(entry.viewers instanceof Set)) entry.viewers = new Set();
  for (const viewer of Array.from(entry.viewers)) {
  ...
  if (entry.controllerWs && !entry.viewers.has(entry.controllerWs)) {
    ...
    entry.controllerClientId = null;
  }
  return entry.viewers;
}

function getSessionControllerClientId(sessionIdOrEntry) {
  const entry = typeof sessionIdOrEntry === 'string'
    ? activeProcesses.get(sessionIdOrEntry)
    : sessionIdOrEntry;
  if (!entry) return null;
  getLiveViewerSet(entry);
  return entry.controllerClientId || null;
}

function getSessionViewerCount(sessionIdOrEntry) {
  const entry = typeof sessionIdOrEntry === 'string'
    ? activeProcesses.get(sessionIdOrEntry)
    : sessionIdOrEntry;
  if (!entry) return 0;
  return getLiveViewerSet(entry).size;
}

function getPreferredSessionController(entry) {
  if (!entry) return null;
  const viewers = getLiveViewerSet(entry);
  if (entry.controllerWs && viewers.has(entry.controllerWs) && entry.controllerWs.readyState === 1) {
    return entry.controllerWs;
  }
  for (const viewer of viewers) {
    if (viewer?.readyState === 1) return viewer;
  }
  return null;
}

function setSessionController(sessionId, ws, clientId, reason = 'unspecified') {
  const entry = activeProcesses.get(sessionId);
  if (!entry) return false;
  const viewers = getLiveViewerSet(entry);
  const nextWs = ws && ws.readyState === 1 ? ws : null;
  const nextClientId = nextWs ? (clientId || wsClientMap.get(nextWs) || null) : null;
  const previousClientId = entry.controllerClientId || null;
  if (nextWs && !viewers.has(nextWs)) viewers.add(nextWs);
  entry.controllerWs = nextWs;
  entry.controllerClientId = nextClientId;
  if (viewers.size > 0) entry.wsDisconnectTime = null;
  invalidateSessionListCache();
  if (previousClientId !== nextClientId) {
    plog('INFO', 'session_controller_changed', {
...
function buildSessionControlSnapshot(sessionId, ws = null) {
  const entry = activeProcesses.get(sessionId);
  if (!entry) {
    return {
      controlState: null,
  ...
  const viewerCount = getSessionViewerCount(entry);
  const controllerClientId = getSessionControllerClientId(entry);
  const currentClientId = ws ? (wsClientMap.get(ws) || null) : null;
  const isViewer = !!(entry && getLiveViewerSet(entry).has(ws));
  const controlState = isViewer && controllerClientId && currentClientId === controllerClientId
    ? 'controller'
...
function buildControlStatePayload(sessionId, ws = null) {
  return {
    type: 'control_changed',
    sessionId,
    ...buildSessionControlSnapshot(sessionId, ws),
  };
}

function broadcastToSessionViewers(sessionId, payloadOrBuilder, options = {}) {
  const entry = activeProcesses.get(sessionId);
  if (!entry) return 0;
  const viewers = getLiveViewerSet(entry);
  let sent = 0;
  for (const viewer of viewers) {
    ...
    const payload = typeof payloadOrBuilder === 'function'
      ? payloadOrBuilder(viewer)
      : payloadOrBuilder;
    if (!payload) continue;
    if (options.excludeWs && viewer === options.excludeWs) continue;
    wsSend(viewer, payload);
    sent += 1;
  }
  return sent;
}

function addSessionViewer(sessionId, ws, clientId) {
  const entry = activeProcesses.get(sessionId);
  if (!entry || !ws) return false;
  const viewers = getLiveViewerSet(entry);
  viewers.add(ws);
  if (clientId) wsClientMap.set(ws, clientId);
  entry.wsDisconnectTime = null;
  const hasController = !!(entry.controllerWs && viewers.has(entry.controllerWs) && entry.controllerWs.readyState === 1);
  const controllerChanged = !hasController
    ? setSessionController(sessionId, ws, clientId, 'viewer_join_initial_controller')
    : false;
  invalidateSessionListCache();
  return controllerChanged;
}

function removeSessionViewer(sessionId, ws, reason = 'viewer_removed') {
  const entry = activeProcesses.get(sessionId);
  if (!entry || !ws) return { removed: false, controllerChanged: false, viewerCount: 0 };
  const viewers = getLiveViewerSet(entry);
  const removed = viewers.delete(ws);
  let controllerChanged = false;
  if (entry.controllerWs === ws) {
    const nextController = getPreferredSessionController(entry);
    controllerChanged = setSessionController(
      sessionId,
      nextController,
      nextController ? (wsClientMap.get(nextController) || null) : null,
      reason
    );
  }
  if (viewers.size === 0) {
  ...
  invalidateSessionListCache();
  return { removed, controllerChanged, viewerCount: viewers.size };
}

function detachWsFromActiveSessionViews(ws, options = {}) {
  const { keepSessionId = null, reason = 'viewer_detached' } = options;
  let changed = false;
  for (const [sessionId] of activeProcesses) {
    if (keepSessionId && sessionId === keepSessionId) continue;
    const result = removeSessionViewer(sessionId, ws, reason);
    if (!result.removed) continue;
    changed = true;
    broadcastToSessionViewers(sessionId, (viewer) => buildControlStatePayload(sessionId, viewer));
  }
  if (changed) broadcastSessionList({ forceRefresh: true });
  return changed;
}
...
function downloadCloudflaredAsset(ws, downloadUrl, assetName) {
  ...
  function completeInstallFromTmp() {
    if (settled) return;
    settled = true;
    if (isTgz) {
  ...
  function failDownload(message) {
    ...
    settled = true;
    cleanupTmp();
    wsSend(ws, { type: 'tunnel_install_progress', done: true, error: message });
  }

  function fallbackToCurl(reason) {
    if (settled || fallbackStarted) return;
    fallbackStarted = true;
    cleanupTmp();
    wsSend(ws, { type: 'tunnel_install_progress', message: `${reason}，正在改用 curl 重试下载...` });
    execFile('curl', [
      '-L',
      '--fail',
      '--connect-timeout', '15',
      ...
      '--retry', '2',
      '--retry-delay', '1',
      '-A', 'webcoding-cloudflared-installer',
      '-o', tmpPath,
      downloadUrl,
    ], { timeout: 330000, maxBuffer: 1024 * 1024 }, (error) => {
      if (error) {
        failDownload(`${reason}；curl 重试失败: ${error.message}`);
        return;
      }
      completeInstallFromTmp();
    });
  }

  function doDownload(url, redirects) {
    if (redirects > 5) {
      fallbackToCurl('Node 下载失败: 重定向过多');
      return;
    }
    const parsedUrl = new URL(url);
    const mod = parsedUrl.protocol === 'https:' ? https : http;
    const req = mod.get(url, { headers: { 'User-Agent': 'webcoding-cloudflared-installer' } }, (res) => {
      if (res.statusCode === 301 || res.statusCode === 302 || res.statusCode === 307 || res.statusCode === 308) {
        res.resume();
        doDownload(new URL(res.headers.location, url).toString(), redirects + 1);
        return;
      }
      if (res.statusCode !== 200) {
        res.resume();
        fallbackToCurl(`Node 下载失败: HTTP ${res.statusCode}`);
        return;
      }
      ...
      fileStream.on('finish', () => {
        fileStream.close(() => completeInstallFromTmp());
      });
      fileStream.on('error', (e) => {
        failDownload('写入失败: ' + e.message);
      });
      res.on('error', (e) => {
        fileStream.destroy();
        fallbackToCurl('Node 下载失败: ' + e.message);
      });
    }).on('error', (e) => {
      fallbackToCurl('Node 下载失败: ' + e.message);
    });
  ...
  for (const f of files) {
    try {
      ...
      sessions.push({
        ...
        viewerCount: getSessionViewerCount(s.id),
        controllerClientId: getSessionControllerClientId(s.id),
...
function broadcastSessionList(options = {}) {
  const sessions = Array.isArray(options.sessions)
    ? options.sessions
    : getSessionListSnapshot(options);
  for (const client of wss.clients) {
    if (client.readyState !== 1 || client.isAuthenticated !== true) continue;
    wsSend(client, { type: 'session_list', sessions });
  }
}
...
function resolveProcessCompletionState(sessionId, entry, exitCode, signal) {
  ...
  const wsConnected = getSessionViewerCount(entry) > 0;
...
function persistProcessCompletionSession(sessionId, entry, pendingSlash) {
  ...
  if (session && (entry.fullText || (entry.toolCalls && entry.toolCalls.length > 0))) {
    ...
    if (getSessionViewerCount(entry) === 0) session.hasUnread = true;
    saveSession(session);
  }
...
function handleConnectedProcessCompletion(sessionId, entry, session, pendingSlash, pendingRetry, contextLimitExceeded, completionError) {
  ...
  const controlMessage = (message) => {
    broadcastToSessionViewers(sessionId, { type: 'system_message', message });
  };
  ...
  if (pendingSlash?.kind === 'compact') {
    ...
    if (autoRetryRequested) {
      if (contextLimitExceeded) {
        ...
        controlMessage('已尝试执行 /compact，但仍未成功解除上下文超限。请手动缩小输入范围后重试。');
      } else {
        controlMessage(compactDoneMessage(entry.agent || 'claude'));
        controlMessage(compactAutoResumeMessage(entry.agent || 'claude'));
  ...
  if (contextLimitExceeded && !pendingSlash && session && getRuntimeSessionId(session)) {
    ...
    if (nextRetryCount > MAX_AUTO_COMPACT_RETRIES) {
      pendingCompactRetries.delete(sessionId);
      controlMessage('自动 /compact 重试已达到上限，请手动缩短输入内容后再试。');
    } else {
      ...
      controlMessage(compactAutoStartMessage(entry.agent || 'claude'));
  ...
  if (completionError && !entry.errorSent && !shouldAutoCompact) {
    ...
    broadcastToSessionViewers(sessionId, { type: 'error', message: completionError });
  }

  broadcastToSessionViewers(sessionId, { type: 'done', sessionId, costUsd: entry.lastCost ?? null });
  broadcastSessionList({ forceRefresh: true });
...
function runProcessCompletionFollowup(sessionId, entry, session, pendingSlash, pendingRetry, contextLimitExceeded, shouldReturnForFollowup, shouldAutoCompact) {
  ...
  const controllerWs = getPreferredSessionController(entry);
  if (shouldReturnForFollowup && controllerWs && session && pendingSlash?.kind === 'compact') {
    const retry = pendingCompactRetries.get(sessionId);
    if (retry?.text) {
      pendingCompactRetries.delete(sessionId);
      handleMessage(controllerWs, { text: retry.text, sessionId, mode: retry.mode || session.permissionMode || 'yolo' });
    }
    return;
  }

  if (shouldAutoCompact && controllerWs && session) {
    pendingSlashCommands.set(sessionId, { kind: 'compact' });
    handleMessage(controllerWs, { text: '/compact', sessionId, mode: session.permissionMode || 'yolo' }, { hideInHistory: true });
  }
}
...
function handleProcessComplete(sessionId, exitCode, signal) {
  ...
  const hasViewers = getSessionViewerCount(entry) > 0;
  const { shouldReturnForFollowup, shouldAutoCompact } = hasViewers
  ...
  for (const [sessionId, entry] of activeProcesses) {
    ...
    if (entry.pid && !isProcessRunning(entry.pid)) {
      plog('INFO', 'pid_monitor_detected_exit', {
        sessionId: sessionId.slice(0, 8),
        pid: entry.pid,
        wsConnected: getSessionViewerCount(entry) > 0,
      });
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === 'OPTIONS') {
    const corsHeaders = applyCorsHeaders(req, {});
    if (corsHeaders['Access-Control-Allow-Origin']) {
      writeHeadWithSecurity(res, 204, corsHeaders);
    } else {
      writeHeadWithSecurity(res, 204, {});
    }
    return res.end();
  }
  ...
  if (req.method === 'GET' && url.pathname === '/health') {
    ...
    return jsonResponseWithCors(req, res, 200, {
      ok: true,
      service: 'webcoding',
      version,
  ...
  if (req.method === 'POST' && url.pathname === '/api/attachments') {
    ...
    if (!token || !hasActiveToken(token)) {
      return jsonResponseWithCors(req, res, 401, { ok: false, message: 'Not authenticated' });
    }
    ...
    if (!IMAGE_MIME_TYPES.has(mime)) {
      return jsonResponseWithCors(req, res, 400, { ok: false, message: '仅支持 PNG/JPG/WEBP/GIF 图片' });
    }
    ...
    req.on('end', () => {
      if (aborted) {
        return jsonResponseWithCors(req, res, 413, { ok: false, message: '图片大小不能超过 10MB' });
      }
      const buffer = Buffer.concat(chunks);
      if (buffer.length === 0) {
        return jsonResponseWithCors(req, res, 400, { ok: false, message: '图片内容为空' });
      }
      const actualMime = detectMimeFromMagic(buffer);
      if (!actualMime || actualMime !== mime) {
        return jsonResponseWithCors(req, res, 400, { ok: false, message: '图片内容与声明类型不一致或文件已损坏' });
      }
      ...
      try {
        ...
        return jsonResponseWithCors(req, res, 500, { ok: false, message: '保存附件失败，请稍后重试' });
      }
    });
    req.on('error', () => {
      if (!res.headersSent) jsonResponseWithCors(req, res, 500, { ok: false, message: '上传过程中断' });
    });
    return;
  }
  ...
  if (req.method === 'GET' && url.pathname === '/api/localfile') {
    const token = extractBearerToken(req);
    if (!token || !hasActiveToken(token)) {
      writeHeadWithSecurity(res, 401, applyCorsHeaders(req, { 'Content-Type': 'text/plain; charset=utf-8' }));
    ...
    if (!rawPath || !path.isAbsolute(rawPath)) {
      writeHeadWithSecurity(res, 400, applyCorsHeaders(req, { 'Content-Type': 'text/plain; charset=utf-8' }));
      return res.end('Invalid path');
    }
    const absPath = path.resolve(rawPath);
    fs.readFile(absPath, 'utf8', (err, data) => {
      if (err) {
        writeHeadWithSecurity(res, 404, applyCorsHeaders(req, { 'Content-Type': 'text/html; charset=utf-8' }));
...
<script>document.getElementById('content').innerHTML=marked.parse(${JSON.stringify(data)});</script>
        ...
        writeHeadWithSecurity(res, 200, applyCorsHeaders(req, { 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'no-cache' }));
        return res.end(htmlContent);
      }
      // Plain text / other text files
      const mime = MIME_TYPES[ext] || 'text/plain; charset=utf-8';
      writeHeadWithSecurity(res, 200, applyCorsHeaders(req, { 'Content-Type': mime, 'Cache-Control': 'no-cache' }));
      res.end(data);
    });
    return;
  }
  ...
  if (req.method === 'DELETE' && url.pathname.startsWith('/api/attachments/')) {
    ...
    if (!id) {
      return jsonResponseWithCors(req, res, 400, { ok: false, message: '缺少附件 ID' });
    }
    removeAttachmentById(id);
    return jsonResponseWithCors(req, res, 200, { ok: true });
  }
...
function isAllowedWsOrigin(origin, req) {
  ...
  return isAllowedAppOrigin(origin, req);
}
  ...
  ws.on('message', (raw) => {
    ...
    switch (msg.type) {
      ...
      case 'get_projects':
        ...
        break;
      case 'get_skill_commands':
        wsSend(ws, { type: 'skill_commands', commands: listInstalledSkillCommands() });
        break;
      case 'save_project':
    ...
    case '/help': {
      ...
      wsSend(ws, {
        ...
        message: agent === 'codex'
          ...
          : base + '\n/model [名称] — 查看/切换模型（支持别名或完整模型 ID）\n/compact — 执行 Claude 原生上下文压缩（保留压缩计划并可自动续跑）\n其他 Claude Code 原生指令将直接转发至 CLI（如 /memory, /effort, /status, /commit 等）',
      });
      break;
    }

    default:
      if (agent === 'claude' && sessionId && session) {
        handleMessage(ws, { text, sessionId: session.id, mode: session.permissionMode || 'yolo' }, { hideInHistory: true });
      } else {
...
function handleNewSession(ws, msg) {
  ...
  saveSession(session);
  detachWsFromActiveSessionViews(ws, { reason: 'new_session_switch' });
  ...
  wsSend(ws, {
    ...
    isRunning: false,
    ...buildSessionControlSnapshot(id, ws),
    ...buildSessionRuntimeMeta(session),
  });
  sendSessionList(ws);
}
...
function handleLoadSession(ws, sessionId) {
  ...
  detachWsFromActiveSessionViews(ws, { keepSessionId: sessionId, reason: 'load_session_switch' });
  wsSessionMap.set(ws, sessionId);
  const clientId = wsClientMap.get(ws) || null;
  if (activeProcesses.has(sessionId)) {
    addSessionViewer(sessionId, ws, clientId);
  }
  ...
  if (activeProcesses.has(sessionId)) {
    ...
    plog('INFO', 'ws_resume_attach', {
      ...
      pid: entry.pid,
      responseLen: (entry.fullText || '').length,
      viewerCount: getSessionViewerCount(entry),
    });
    broadcastToSessionViewers(sessionId, (viewer) => buildControlStatePayload(sessionId, viewer));
    broadcastSessionList({ forceRefresh: true });
    wsSend(ws, {
...
function execSqliteScriptWithNodeSqlite(dbPath, script) {
  ...
  try {
    ...
    plog('WARN', 'sqlite_node_cleanup_failed', { dbPath, error: error.message });
    return false;
  } finally {
...
async function execSqliteScriptQuiet(dbPath, script) {
  const cliResult = await execFileQuiet('sqlite3', [dbPath, script]);
  if (cliResult.ok) return true;
  return execSqliteScriptWithNodeSqlite(dbPath, script);
}
...
async function deleteCodexLocalSession(threadId, importedRolloutPath = null) {
  ...
  try {
    ...
    if (await execSqliteScriptQuiet(CODEX_STATE_DB_PATH, stateSql)) {
      removedDbRows = true;
    }

    if (fs.existsSync(CODEX_LOG_DB_PATH)) {
      await execSqliteScriptQuiet(CODEX_LOG_DB_PATH, `DELETE FROM logs WHERE thread_id = ${quotedThreadId};`);
    }
  } catch {}
...
function handleDeleteSession(ws, sessionId) {
  ...
  if (activeProcesses.has(sessionId)) {
    ...
    if (entry.tailer) entry.tailer.stop();
    broadcastToSessionViewers(sessionId, { type: 'done', sessionId });
    removeActiveProcess(sessionId);
  }
  cleanRunDir(sessionId);
  try {
...
function handleDisconnect(ws, wsId) {
  ...
  for (const [sid, entry] of activeProcesses) {
    const result = removeSessionViewer(sid, ws, 'disconnect');
    if (!result.removed) continue;
    affectedSessions.push({
      sessionId: sid.slice(0, 8),
      pid: entry.pid,
      viewerCount: result.viewerCount,
      controllerClientId: entry.controllerClientId || null,
    });
    broadcastToSessionViewers(sid, (viewer) => buildControlStatePayload(sid, viewer));
  }
...
function handleDetachView(ws) {
  detachWsFromActiveSessionViews(ws, { reason: 'detach_view' });
  wsSessionMap.delete(ws);
}
...
function handleAbort(ws) {
  ...
  if (!entry) return;
  if (entry.controllerWs !== ws) {
    setSessionController(sessionId, ws, wsClientMap.get(ws) || null, 'abort_takeover');
    broadcastToSessionViewers(sessionId, (viewer) => buildControlStatePayload(sessionId, viewer));
    broadcastSessionList({ forceRefresh: true });
  }
...
function handleMessage(ws, msg, options = {}) {
  ...
  if (sessionId && activeProcesses.has(sessionId)) {
    const activeEntry = activeProcesses.get(sessionId);
    wsSessionMap.set(ws, sessionId);
    if (activeEntry && activeEntry.controllerWs !== ws) {
      setSessionController(sessionId, ws, wsClientMap.get(ws) || null, 'message_takeover');
      broadcastToSessionViewers(sessionId, (viewer) => buildControlStatePayload(sessionId, viewer));
      broadcastSessionList({ forceRefresh: true });
    }
  ...
  detachWsFromActiveSessionViews(ws, { keepSessionId: currentSessionId, reason: 'message_session_switch' });
  ...
  if (!sessionId) {
    wsSend(ws, {
      ...
      historyPending: false,
      isRunning: false,
      ...buildSessionControlSnapshot(currentSessionId, ws),
      ...buildSessionRuntimeMeta(session),
    });
  }
  sendSessionList(ws);
  ...
  entry = {
    ...
    viewers: new Set([ws]),
    controllerWs: ws,
    controllerClientId: wsClientMap.get(ws) || null,
    agent: getSessionAgent(session),
    cwd: spawnSpec.cwd,
  ...
  broadcastToSessionViewers(currentSessionId, (viewer) => buildControlStatePayload(currentSessionId, viewer));
  broadcastSessionList({ forceRefresh: true });
...
const {
  ...
  wsSend,
  broadcastSessionMessage: (sessionId, payload) => broadcastToSessionViewers(sessionId, payload),
  truncateObj,
  sanitizeToolInput,
  loadSession,
  saveSession,
...
function handleImportNativeSession(ws, msg) {
  ...
  saveSession(session);
  detachWsFromActiveSessionViews(ws, { reason: 'import_native_session_switch' });
  wsSessionMap.set(ws, id);
  wsSend(ws, {
    type: 'session_info',
    sessionId: id,
...
function handleImportCodexSession(ws, msg) {
  ...
  saveSession(session);
  detachWsFromActiveSessionViews(ws, { reason: 'import_codex_session_switch' });
  wsSessionMap.set(ws, id);
  wsSend(ws, {
    type: 'session_info',
    sessionId: id,
...
setInterval(() => {
  ...
  for (const [sid, entry] of activeProcesses) {
    ...
    procs.push({
      ...
      wsConnected: getSessionViewerCount(entry) > 0,
      viewerCount: getSessionViewerCount(entry),
      controllerClientId: getSessionControllerClientId(entry),
      wsDisconnectTime: entry.wsDisconnectTime || null,
      responseLen: (entry.fullText || '').length,
    });
  }
